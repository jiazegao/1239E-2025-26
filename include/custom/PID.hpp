...
class Lever_PID {
    public:
        Lever_PID(pros::Motor* motor, pros::Rotation* sensor, double kP, double kI, double kD, double errorRange, double errorTimeout, double maxReverseSpeed, double maxScoreSpeed, bool autoReset = true, LEVER_STAGE* leverStage = nullptr)
            : motor(motor), sensor(sensor), kP(kP), kI(kI), kD(kD), errorRange(errorRange), errorTimeout(errorTimeout), maxReverseSpeed(maxReverseSpeed), maxScoreSpeed(maxScoreSpeed), prevError(0), integral(0), target(0), autoReset(autoReset), leverStage(leverStage) { timer.hardReset(errorTimeout); motor->set_brake_mode(pros::E_MOTOR_BRAKE_HOLD); tickTimer.hardReset(1000); } // Constructor
        ...
        void setTarget(double newTarget, double maxReverseSpeed_ = -300, double maxScoreSpeed_ = 300) {
            target = newTarget;
            ...
            previousValue = sensor->get_position()/100.0;
        }
