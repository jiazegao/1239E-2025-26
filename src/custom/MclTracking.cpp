...
void MclTracking::computeSensorPolars() {
    auto setPolar = [](SensorGeo& s){
        s.offsetDistIn   = std::hypot(s.horizOffsetIn, s.vertOffsetIn);
        s.offsetAngleDeg = std::fmod( std::atan2(s.vertOffsetIn, s.horizOffsetIn) * 180.0 / M_PI + 360.0, 360.0 );
    };
    setPolar(backS);
    setPolar(rightS);
    setPolar(leftS);
...
void MclTracking::motionUpdate(double dx_in, double dy_in, double dtheta_rad) {
    ...
    for (auto& p : particles) {
        ...
        p.x = std::clamp(p.x, -FIELD_HALF - 5.0, FIELD_HALF + 5.0);
        p.y = std::clamp(p.y, -FIELD_HALF - 5.0, FIELD_HALF + 5.0);
    }
}
...
double MclTracking::simulateDistance(const MclParticle& p, const SensorGeo& sg) const {
    ...
    const double offThetaRad   = deg2rad( sg.offsetAngleDeg - botHeadingDeg );
    const double sx = p.x + std::cos(offThetaRad) * sg.offsetDistIn;
    const double sy = p.y + std::sin(offThetaRad) * sg.offsetDistIn;

    const double rayDeg = std::fmod(botHeadingDeg + sg.mainAngleDeg + 360.0, 360.0);
